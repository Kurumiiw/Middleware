...
class Controller:
    ...
    def run_simultaneous(self):
        ...
        while any([worker.is_alive() for worker in self.workers.values()]):
            ...
            for worker in self.workers.values():
                print(
                    f"Progress {worker.name}: {(worker.total_packets-worker.progress)/worker.total_packets*100:.2f}%"
                )
    ...
    def run_single(self, name):
        ...
        while self.workers[name].is_alive():
            if self.workers[name].reliable:
                try:
                    ...
                    print(
                        f"Progress {name}: {(self.workers[name].total_packets-self.workers[name].progress)/self.workers[name].total_packets*100:.2f}%",
                        end="\r",
                    )
        ...
        print(
            f"Overhead bytes: {self.workers[name].statistics['send_bytes']-self.workers[name].statistics['data_bytes']}"
        )
        print(
            f"Overhead ratio: {(self.workers[name].statistics['send_bytes']-self.workers[name].statistics['data_bytes'])/self.workers[name].statistics['send_bytes']*100:.2f}%"
        )
