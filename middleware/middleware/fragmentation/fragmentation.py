...
class Fragmenter:
    ...
    def fragment(self, data: bytes) -> list[bytes]:
        if len(data) > max_dgram_payload:
            raise ValueError("Payload is too large to be sent with a MiddlewareUnreliable datagram")
        ...
        for frag_idx in range(frag_count):
            ...
            fragment.extend(data[frag_idx*max_frag_payload:(frag_idx+1)*max_frag_payload])
...
class Reassembler:
    ...
    def timeout_old_datagrams(self) -> None:
        ...
        for key in list(self.datagrams.keys()):
            if (time.perf_counter() - self.datagrams[key].timestamp) >= config.fragment_timeout:
    ...
    def check_for_completed_datagrams(self) -> tuple[bytes, tuple[str, int]]:
        ...
        for key in self.datagrams:
            if self.datagrams[key].is_fin and len(self.datagrams[key].frag_store.keys()) == self.datagrams[key].expected_frag_count:
                frag_store = self.datagrams[key].frag_store
                addr = key[0]
                data = bytearray(b"".join([frag_store[i] for i in range(len(frag_store.keys()))]))
